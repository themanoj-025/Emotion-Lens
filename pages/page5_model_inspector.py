...
def _render_feature_maps_tab(model) -> None:
    ...
    st.markdown(
        f"Shape: {activations.shape} — {activations.shape[-1]} filters of size {activations.shape[0]}×{activations.shape[1]}"
    )
...
def _render_gradcam_tab(model) -> None:
    ...
    from utils.emotion_utils import compute_gradcam, predict_emotion

    # Use grayscale face ROI directly
    emotion, confidence, probs = predict_emotion(model, img_resized)

    st.markdown(
        f"**Prediction:** {EMOTION_CONFIG.get(emotion, {}).get('emoji', '')} {emotion} ({confidence * 100:.1f}%)"
    )
