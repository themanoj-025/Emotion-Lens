            ...
            if cap.isOpened():
                ...
                if ret:
                    ...
                    if detected_emotion and detected_emotion == target:
                        ...
                        st.warning(
                            f"Detected: {EMOTION_CONFIG.get(detected_emotion, {}).get('emoji', '')} {detected_emotion} — not a match! Need: {EMOTION_CONFIG[target]['emoji']} {target}"
                        )
                        st.session_state.game_history.append(
                            {
                                "mode": "Make This Face!",
...
def _render_guess_emotion_mode(model) -> None:
    ...
    st.info(
        "Upload a face image. Try to guess the emotion yourself, then compare with what the CNN predicts!"
    )
        ...
        if user_guess:
            ...
            if results:
                ...
                if is_correct:
                    ...
                    st.info(
                        f"💡 The AI detected {ai_config['emoji']} {ai_emotion}. Look at the {ai_emotion.lower()} facial features — the mouth, eyes, and brow area."
                    )
